...
// (p - 1) / 2
const P_1_2: ArrEd25519FieldElement = Array(secret_array!(
    U64,
    [
...
// (p + 3) / 8
const P_3_8: ArrEd25519FieldElement = Array(secret_array!(
    U64,
    [
...
// (p - 5) / 8
const P_5_8: ArrEd25519FieldElement = Array(secret_array!(
    U64,
    [
        0x0fffffffffffffffu64,
        0xffffffffffffffffu64,
...
pub fn expand_message_xmd(msg: &ByteSeq, dst: &ByteSeq, len_in_bytes: usize) -> ByteSeqResult {
...
pub fn ed_hash_to_field(msg: &ByteSeq, dst: &ByteSeq, count: usize) -> SeqEdResult {
    ...
    for i in 0..count {
        ...
        let u_i = Ed25519FieldElement::from_byte_seq_be(
            &EdFieldHash::from_byte_seq_be(&tv)
                .to_byte_seq_be()
                .slice(32, 32),
        ); // OS2IP(tv) mod p
        output[i] = u_i;
...
pub fn ed_encode_to_curve(msg: &ByteSeq, dst: &ByteSeq) -> EdPointResult {
...
mod tests {
    ...
    fn point_on_curve(msg: Wrapper) -> bool {
        let dst = ByteSeq::from_public_slice(b"ECVRF_edwards25519_XMD:SHA-512_ELL2_NU_");
    ...
    fn test_with_vector(msg: ByteSeq, px: &str, py: &str, u: &str, qx: &str, qy: &str) {
        let one = Ed25519FieldElement::ONE();
        let dst =
            ByteSeq::from_public_slice(b"QUUX-V01-CS02-with-edwards25519_XMD:SHA-512_ELL2_NU_");
        ...
        let (pxt, pyt, pzt, _) =
            ed_clear_cofactor((qx_normal, qy_normal, one, qx_normal * qy_normal));
