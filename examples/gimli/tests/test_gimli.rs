...
// The tests in the below function are not edge cases and have been generated randomly through the python implementation of Gimli
// Currently, the 'expected' variable is the output that is given by python-gimli and 'out' is given by gimli-rust for the same input

// For some reason, the expected and out's are not matching for all 3 of the inputs listed below

#[test]
fn tg_gimli_hash() {
    // Test 1 (16 length input)
    let input = ByteSeq::from_public_slice(&[0x11, 0x60, 0x65, 0x50, 0x0, 0x99, 0x6d, 0xf4, 0x70, 0x88, 0xd9, 0x49, 0x95, 0x83, 0x75, 0x7a]);
    let expected = Digest::from_public_slice(&[
        0x92, 0x8f, 0xbd, 0xa3, 0x8a, 0x4, 0x9e, 0xa5, 
        0xd5, 0xa6, 0xa0, 0x8d, 0x16, 0x1b, 0xeb, 0xb3,
        0x41, 0x3d, 0xde, 0xa3, 0x4e, 0xad, 0xb1, 0xdd,
        0x93, 0x91, 0x6a, 0x58, 0xa6, 0x36, 0x7, 0xb7,
    ]);

    let out = gimli_hash(&input);
    assert_secret_array_eq!(expected, out, U8);

    // Test 2 (16 length input)
...
/*
    Python code to generate random test cases:
