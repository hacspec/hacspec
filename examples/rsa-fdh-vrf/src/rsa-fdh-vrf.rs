...
#[rustfmt::skip]
const ONE: IntByte = Array(secret_array!(U8,  [0x01u8]));
#[rustfmt::skip]
const TWO: IntByte = Array(secret_array!(U8,  [0x02u8]));
...
fn vrf_mgf1(n: RSAInt, alpha: &ByteSeq) -> ByteSeqResult {
    ...
    let mgf_string = SUITE_STRING.concat(&ONE).concat(&mgf_salt).concat(alpha);
...
mod tests {
    ...
    fn test_neg_rsafdhvrf() {
        fn neg_rsafdhvrf(kp: Keyp, fake: Keyp, alpha: Wrapper) -> bool {
            ...
            match verify((fake.n, fake.e), &alpha, &pi) {
                ...
                Err(e) => matches!(e, Error::VerificationFailed | Error::MessageTooLarge),
            }
        }
        QuickCheck::new()
            .tests(NUM_TESTS)
    ...
    fn test_neg_alpha_rsafdhvrf() {
        fn neg_alpha_rsafdhvrf(kp: Keyp, alpha: Wrapper, fake_alpha: Wrapper) -> bool {
            let alpha = i2osp(alpha.0, BYTE_SIZE).unwrap();
        ...
        QuickCheck::new()
            .tests(NUM_TESTS)
            .quickcheck(neg_alpha_rsafdhvrf as fn(Keyp, Wrapper, Wrapper) -> bool);
    }

    // Test vector generation
    // Strings should be given in hexadecimal
    fn generate_test_vector(alpha: &str, kp: Keyp) -> Result<(String, String), Error> {
    ...
    fn test_empty() {
        ...
        let beta =
            ByteSeq::from_hex("d065ca3be8716236e99f64139adf481090f0a0c839f86ffda3c4fad948166af0");
    ...
    fn test_72() {
        ...
        let beta =
            ByteSeq::from_hex("a229210b84f0bb43b296075f226dee433cf2727cd6c2e4871afdeb77414f6a47");
    ...
    fn test_af82() {
        ...
        let beta =
            ByteSeq::from_hex("ebc5582b6aaf23c424ec1c74e1b8250327c957967fa37566284dac8400e62032");
