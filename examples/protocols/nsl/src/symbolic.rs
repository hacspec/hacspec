...
impl Env for Seq<Entry> {
    type SessionId = usize;
    type MessageId = usize;
    
    fn rand_gen(len:usize,env:&mut Self) -> Bytes {
        let mut rnd = Seq::new(len);
        rnd[0] = U8(env.len() as u8);
        rnd[1] = U8((env.len() >> 8) as u8);
        rnd[2] = U8((env.len() >> 16) as u8);
        rnd[2] = U8((env.len() >> 24) as u8);
        *env = env.push(&Entry::Fresh{rnd:rnd.clone()});
        rnd
    }

    fn trigger_event(a:Principal,ev:Bytes,env:&mut Self) {
        *env = env.push(&Entry::Event{a,ev});
    }

    fn new_session(a:Principal,st:Bytes,env:&mut Self) -> usize{
        let sid = env.len();
        *env = env.push(&Entry::Session{a,sid,st});
        sid
    }
