...
fn empty_message() {
    let msg = ByteSeq::from_public_slice(&[]);
    let k = PolyKey::from_public_slice(&[
        0xa0, 0xf3, 0x08, 0x00, 0x00, 0xf4, 0x64, 0x00, 0xd0, 0xc7, 0xe9, 0x07, 0x6c, 0x83, 0x44,
        0x03, 0xdd, 0x3f, 0xab, 0x22, 0x51, 0xf1, 0x1a, 0xc7, 0x59, 0xf0, 0x88, 0x71, 0x29, 0xcc,
        0x2e, 0xe7,
    ]);
    let expected = Tag::from_public_slice(&[
        0xdd, 0x3f, 0xab, 0x22, 0x51, 0xf1, 0x1a, 0xc7, 0x59, 0xf0, 0x88, 0x71, 0x29, 0xcc, 0x2e,
        0xe7,
    ]);
    let computed = poly1305(&msg, k);
    assert!(Tag::declassify_eq(&expected, &computed))
...
#[test]
fn short_message() {
    let msg = ByteSeq::from_public_slice(&[0x02, 0xfc]);
    let k = PolyKey::from_public_slice(&[
    ...
    let expected = Tag::from_public_slice(&[
        0x06, 0x12, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c, 0x0c,
        0x0c,
    ]);
    let computed = poly1305(&msg, k);
    println!("tag:      {:x?}", computed);
