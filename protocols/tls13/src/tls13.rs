...
fn read_bytes(stream:&mut TcpStream,buf:&mut [u8], nbytes:usize) -> Res<usize> {
    match stream.read(&mut buf[..]) {
        Ok(len) => {
            if len >= nbytes {Ok(len-nbytes)}
            else {read_bytes(stream, &mut buf[len..], nbytes - len)}
        },
...
fn read_record(stream:&mut TcpStream,buf:&mut [u8]) -> Res<usize> {
   ...
   let l1 = b[4] as usize;
   let len = l0 * 256 + l1;
   if len + 5 > buf.len() {Err(parse_failed)}
   else {
       let extra = read_bytes(stream,&mut buf[0..len+5],len+5)?;
       if extra > 0 {Err(parse_failed)}
       else {Ok(len+5)}}
}
...
fn put_record(stream:&mut TcpStream,rec:&Bytes) -> Res<()> {
    ...
    match stream.write(&wire) {
        Err(_) => Err(parse_failed),
        Ok(len) => {if len < wire.len() {Err(parse_failed)}
                   else {Ok(())}}}
}
...
fn decrypt_handshake_flight(stream:&mut TcpStream,buf:&mut [u8],cipherH:DuplexCipherStateH) -> Res<(Bytes,DuplexCipherStateH)> {
...
fn decrypt_tickets_and_data(stream:&mut TcpStream,buf:&mut [u8],cipher1:DuplexCipherState1) -> Res<(Bytes,DuplexCipherState1)> {
    let mut payload = Bytes::new(0);
    let mut data = false;
    ...
    while !data {
        ...
        else {println!("Received Session Ticket");}
    }
    Ok((payload,cipher1))
}
...
fn main () {
    ...
    let host = if args.len() <= 1 {"www.google.com"} else {&args[1]};
