...
fn time<F, R>(mut f: F, acc: &mut f64) -> R
where
    F: FnMut() -> R,
{
    let start = Instant::now();
    ...
    CF: FnMut(&[u8]),
{
    ...
    for _ in 0..rounds {
        let stream = time(|| server_send(&buf), &mut time_send);
        let _ = time(|| client_receive(&stream), &mut time_recv);
    }

    (rounds, time_send, time_recv)
}

fn bench_bulk_rustls(params: &BenchmarkParam, plaintext_size: u64, mtu: Option<usize>) {
    ...
    let (rounds, time_send, time_recv) = bench_bulk(
        plaintext_size,
        |buf| {
        ...
        "bulk" => match args.next() {
            Some(suite) => {
                ...
                for param in lookup_matching_benches(&suite).iter() {
                    bench_bulk_rustls(param, len, mtu);
                }
            }
            None => {
...
fn all_tests() {
    for test in ALL_BENCHMARKS.iter() {
        bench_bulk_rustls(test, 1024 * 1024, None);
        bench_bulk_rustls(test, 1024 * 1024, Some(10000));
