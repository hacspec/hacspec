...
pub(crate) fn pad<T: Numeric>(v: &[T], l: usize) -> Vec<T> {
...
pub(crate) fn make_fixed_length<T: Numeric>(v: &[T], l: usize) -> Vec<T> {
...
pub(crate) fn normalize<T: Numeric>(x: &[T], y: &[T]) -> (Vec<T>, Vec<T>) {
...
pub(crate) fn is_zero<T: Numeric>(v: &[T]) -> bool {
    for &x in v {
...
/// coefficients in `v`.
pub(crate) fn poly_z_inv<T: Numeric>(v: &[T], n: T) -> Vec<T> {
