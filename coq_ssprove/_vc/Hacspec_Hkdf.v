...
Program Definition extract
  : both_package (CEfset ([salt_or_zero_898_loc])) [interface
  #val #[ HMAC ] : hmac_inp → hmac_out ] [(EXTRACT,(
      extract_inp,extract_out))] :=
  let temp_package_both := (fun temp_inp => 
    let '(salt_900 , ikm_901) := temp_inp : byte_seq '× byte_seq in
    ...
    ((letbm salt_or_zero_898 : seq uint8 loc( salt_or_zero_898_loc ) :=
        ...
        letb 'salt_or_zero_898 :=
          ...
          then lift_scope (L1 := CEfset (
            [salt_or_zero_898_loc])) (L2 := CEfset (
            [salt_or_zero_898_loc])) (I1 := [interface]) (I2 := [interface
        ...
        ) : both (CEfset ([salt_or_zero_898_loc])) [interface
      #val #[ HMAC ] : hmac_inp → hmac_out ] (prk_t)))in
  both_package' _ _ (EXTRACT,(extract_inp,extract_out)) temp_package_both.
...
Program Definition build_hmac_txt
  : both_package (CEfset ([out_903_loc])) [interface] [(BUILD_HMAC_TXT,(
      build_hmac_txt_inp,build_hmac_txt_out))] :=
  let temp_package_both := (fun temp_inp => 
    let '(
      t_905 , info_906 , iteration_907) := temp_inp : byte_seq '× byte_seq '× uint8 in
    ...
    ((letbm out_903 : seq uint8 loc( out_903_loc ) :=
        ...
        ) : both (CEfset ([out_903_loc])) [interface] (byte_seq)))in
  both_package' _ _ (BUILD_HMAC_TXT,(
      build_hmac_txt_inp,build_hmac_txt_out)) temp_package_both.
Fail Next Obligation.
...
Program Definition div_ceil
  : both_package (CEfset ([q_909_loc])) [interface] [(DIV_CEIL,(
      div_ceil_inp,div_ceil_out))] :=
  let temp_package_both := (fun temp_inp => 
    let '(a_911 , b_912) := temp_inp : uint_size '× uint_size in
    ...
    ((letbm q_909 : uint_size loc( q_909_loc ) :=
        ...
        letb 'q_909 :=
          ...
          then lift_scope (L1 := CEfset ([q_909_loc])) (L2 := CEfset (
            [q_909_loc])) (I1 := [interface]) (I2 := [interface]) (H_loc_incl := _) (H_opsig_incl := _) (
        ...
        ) : both (CEfset ([q_909_loc])) [interface] (uint_size)))in
  both_package' _ _ (DIV_CEIL,(div_ceil_inp,div_ceil_out)) temp_package_both.
Fail Next Obligation.
...
Program Definition check_output_limit
  ...
  #val #[ DIV_CEIL ] : div_ceil_inp → div_ceil_out ] [(CHECK_OUTPUT_LIMIT,(
      check_output_limit_inp,check_output_limit_out))] :=
  let temp_package_both := (fun temp_inp => 
    let '(l_914) := temp_inp : uint_size in
    
    let div_ceil := fun x_0 x_1 => package_both div_ceil (x_0,x_1) in
    ((letb n_915 : uint_size :=
      ...
      #val #[ DIV_CEIL ] : div_ceil_inp → div_ceil_out ] ((result (
            hkdf_error_t) (uint_size)))))in
  both_package' _ _ (CHECK_OUTPUT_LIMIT,(
      check_output_limit_inp,check_output_limit_out)) temp_package_both.
Fail Next Obligation.
...
Program Definition expand
  : both_package (CEfset ([t_i_917_loc ; t_918_loc])) [interface
  ...
  #val #[ HMAC ] : hmac_inp → hmac_out ] [(EXPAND,(expand_inp,expand_out))] :=
  let temp_package_both := (fun temp_inp => 
    let '(
      prk_926 , info_924 , l_921) := temp_inp : byte_seq '× byte_seq '× uint_size in
    
    let build_hmac_txt := fun x_0 x_1 x_2 => package_both build_hmac_txt (
      x_0,x_1,x_2) in
    let check_output_limit := fun x_0 => package_both check_output_limit (
      x_0) in
    let hmac := fun x_0 x_1 => package_both hmac (x_0,x_1) in
    ((letbnd(_) n_922 : uint_size := check_output_limit (lift_to_both0 l_921) in
        ...
        letb '(t_i_917, t_918) :=
          foldi_both' (lift_to_both0 (usize 0)) (lift_to_both0 n_922) prod_ce(
              t_i_917,
              t_918
            ) (L := (CEfset ([t_i_917_loc ; t_918_loc]))) (I := ([interface
              #val #[ BUILD_HMAC_TXT ] : build_hmac_txt_inp → build_hmac_txt_out ;
              #val #[ CHECK_OUTPUT_LIMIT ] : check_output_limit_inp → check_output_limit_out ;
              #val #[ HMAC ] : hmac_inp → hmac_out ])) (fun i_923 '(
              t_i_917,
              t_918
            ) =>
        ...
        ) : both (CEfset ([t_i_917_loc ; t_918_loc])) [interface
      #val #[ BUILD_HMAC_TXT ] : build_hmac_txt_inp → build_hmac_txt_out ;
      #val #[ CHECK_OUTPUT_LIMIT ] : check_output_limit_inp → check_output_limit_out ;
      #val #[ HMAC ] : hmac_inp → hmac_out ] (hkdf_byte_seq_result_t)))in
  both_package' _ _ (EXPAND,(expand_inp,expand_out)) temp_package_both.
Fail Next Obligation.
