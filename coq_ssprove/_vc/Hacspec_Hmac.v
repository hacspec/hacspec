...
Program Definition k_block
  : both_package (fset.fset0) [interface #val #[ HASH ] : hash_inp → hash_out
  ] [(K_BLOCK,(k_block_inp,k_block_out))] :=
  let temp_package_both := (fun temp_inp => 
    let '(k_928) := temp_inp : byte_seq in
    ...
    ((lift_scope (H_loc_incl := _) (H_opsig_incl := _) (
          ...
          then array_update_start (array_new_ (default : uint8) (block_len_v)) (
            ...
            lift_to_both0 k_928))
        ) : both (fset.fset0) [interface #val #[ HASH ] : hash_inp → hash_out
      ] (block_t)))in
  both_package' _ _ (K_BLOCK,(k_block_inp,k_block_out)) temp_package_both.
...
Program Definition hmac
  : both_package (CEfset ([h_in_930_loc ; h_in_931_loc])) [interface
  #val #[ HASH ] : hash_inp → hash_out ;
  #val #[ K_BLOCK ] : k_block_inp → k_block_out ] [(HMAC,(
      hmac_inp,hmac_out))] :=
  let temp_package_both := (fun temp_inp => 
    let '(k_934 , txt_936) := temp_inp : byte_seq '× byte_seq in
    
    let hash := fun x_0 => package_both hash (x_0) in
    let k_block := fun x_0 => package_both k_block (x_0) in
    ((letb k_block_935 : block_t := k_block (lift_to_both0 k_934) in
        ...
        ) : both (CEfset ([h_in_930_loc ; h_in_931_loc])) [interface
      #val #[ HASH ] : hash_inp → hash_out ;
      #val #[ K_BLOCK ] : k_block_inp → k_block_out ] (prk_t)))in
  both_package' _ _ (HMAC,(hmac_inp,hmac_out)) temp_package_both.
Fail Next Obligation.
