...
Definition k_block (k_0 : byte_seq) : block_t :=
  (if ((seq_len (k_0)) >.? (block_len_v)):bool then (array_update_start (
        array_new_ (default) (block_len_v)) (array_to_seq (hash (k_0)))) else (
...
Definition hmac (k_1 : byte_seq) (txt_2 : byte_seq) : prk_t :=
  ...
  let h_in_4 : seq uint8 :=
    seq_from_seq (array_to_seq ((k_block_3) array_xor (i_pad_v))) in 
  let h_in_4 :=
    seq_concat (h_in_4) (txt_2) in 
  let h_inner_5 : sha256_digest_t :=
    hash (h_in_4) in 
  let h_in_6 : seq uint8 :=
    seq_from_seq (array_to_seq ((k_block_3) array_xor (o_pad_v))) in 
  let h_in_6 :=
    seq_concat (h_in_6) (array_to_seq (h_inner_5)) in 
  array_from_seq (hash_size_v) (array_to_seq (hash (h_in_6))).
