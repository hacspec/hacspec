...
Proof.
  ...
  apply (r_transL
           (P := (fun '(h₀, h₁) => h₀ = h₁))
           (#put r_1_loc := x ;;
            #put r_1_loc := @mkWord (S nat63) (Z.lxor x y) (@wxor_subproof (S nat63) x y) ;;
            r_0 ← get r_1_loc ;;
            @ret (word_choiceType U64) r_0)
        ).  
  apply (@r_put_get _ r_1_loc x (fun (r_1 : (S nat63).-word) =>
                            (#put r_1_loc := @mkWord (S nat63) (Z.lxor r_1 y) (@wxor_subproof (S nat63) r_1 y) ;;
                                             r_0 ← get r_1_loc ;;
                                             @ret (word_choiceType U64) r_0 ))).
  ...
  apply (r_transL
           (#put r_1_loc := @mkWord (S nat63) (Z.lxor x y) (@wxor_subproof (S nat63) x y) ;;
            r_0 ← get r_1_loc ;;
            @ret (word_choiceType U64) r_0 )).
  apply (contract_put _ r_1_loc x (@mkWord (S nat63) (Z.lxor x y) (@wxor_subproof (S nat63) x y))).
  

  apply (r_transL (#put r_1_loc := @mkWord (S nat63) (Z.lxor x y) (@wxor_subproof (S nat63) x y) ;; @ret (word_choiceType U64) (@mkWord (S nat63) (Z.lxor x y) (@wxor_subproof (S nat63) x y)))).  
  apply r_put_get.
