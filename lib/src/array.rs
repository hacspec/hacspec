...
#[derive(Clone, Copy)]
pub struct Array<T: Default + Copy, const S: usize>(pub [T; S]);
...
impl<T: Default + Copy, const S: usize> Index<usize> for Array<T, S> {
    type Output = T;
    #[cfg_attr(feature = "use_attributes", unsafe_hacspec(Array<T,S>))]
...
impl<T: Default + Copy, const S: usize> IndexMut<usize> for Array<T, S> {
    #[cfg_attr(feature = "use_attributes", unsafe_hacspec($name))]
...
impl<T: Default + Copy + PartialEq, const S: usize> PartialEq for Array<T, S> {
    #[cfg_attr(feature = "use_attributes", unsafe_hacspec(Array<T, S>))]
...
macro_rules! _secret_array {
    ($name:ident,$l:expr,$t:ty, $tbase:ty) => {
        ...
        // }
        /// **Warning:** declassifies secret integer types.
        // TODO: is that useful? don't we want to write `v1.declassify() == v2.declassify()` instead of `v1.declassify_eq(v2)`?
        // the former would be more composable, e.g. `v1.declassify() == v2`
        // impl $name {
        //     #[cfg_attr(feature = "use_attributes", unsafe_hacspec($name))]
        //     pub fn declassify_eq(&self, other: &Self) -> bool {
        //         self.0[..]
