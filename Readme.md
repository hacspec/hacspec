...
The compiler version is currently pinned to `nightly-2021-06-01`.
...
```bash
cd language
rustup toolchain install nightly-2021-06-01
rustup component add --toolchain nightly-2021-06-01 rustc-dev
cargo +nightly-2021-06-01 install hacspec
```

Depending on your system you might also need `llvm-tools-preview`

```bash
rustup component add --toolchain nightly-2021-06-01 llvm-tools-preview
```

**Usage**

In a hacspec crate or workspace directory typechecking can be done as follows now:
(Specifying `+nightly-2021-06-01` is only necessary if it's not specified in the toolchain as it is in this main repository.)

```bash
cargo +nightly-2021-06-01 hacspec <crate-name>
```

Note that the crate dependencies need to be compiled before it can be typechecked.

```bash
cargo +nightly-2021-06-01 build
```
...
```bash
cargo +nightly-2021-06-01 hacspec -o <fst-name>.fst <crate-name>
cargo +nightly-2021-06-01 hacspec -o <ec-name>.ec <crate-name>
```
