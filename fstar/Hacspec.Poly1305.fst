...
let encode_r (r_1864 : block) : field_element =
  let r_128_1865 = RSeq.from_slice 16 (r_1864) (usize 0) (blocksize) in
...
let encode (block_1868 : byte_seq) : field_element =
  let block_len_1869 = RSeq.len (block_1868) in
  let block_as_u128_1870 =
    RSeq.from_slice 16 (block_1868) (usize 0) (min (usize 16) (block_len_1869))
  in
  let w_elem_1871 =
...
let poly_inner (m_1873 : byte_seq) (r_1874 : field_element) : field_element =
  ...
  let (acc_1875) =
    foldi (usize 0) (RSeq.num_chunks (m_1873) (blocksize)) (fun (
        i_1876,
        (acc_1875)
      ) ->
      let (_, block_1877) = RSeq.get_chunk (m_1873) (blocksize) (i_1876) in
  ...
  let s_elem_1880 =
    RNat.from_secret_literal (
      uint128_from_le_bytes (
        RSeq.from_slice 16 (key_1879) (blocksize) (blocksize)))
  in
  let r_elem_1881 =
    encode_r (RSeq.from_slice_range blocksize (key_1879) ((usize 0, blocksize)))
  in
  let a_1882 = poly_inner (m_1878) (r_elem_1881) in
  let n_1883 = a_1882 + s_elem_1880 in
  let n_v_1884 = RNat.to_public_byte_seq_le (n_1883) in
  let tag_1885 = RSeq.new_ blocksize () in
  let (tag_1885) =
    foldi (usize 0) (
        min (RSeq.len (tag_1885)) (RSeq.len #pub_uint8 (n_v_1884))) (fun (
        i_1886,
        (tag_1885)
      ) ->
      let tag_1885 =
