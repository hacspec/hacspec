...
let poly1305_encode_last
      (pad_len_5: block_index{(**) pad_len_5 <= 16})
      (b_6: sub_block{(**) Seq.length b_6 = pad_len_5})
    :
    field_element =
  let n_7 =
  ...
  (**) Math.Lemmas.pow2_le_compat 128 (8 * pad_len_5);
...
let poly1305_update_block (b_11: poly_block) (st_12: poly_state) : poly_state =
  let acc_13, r_14, k_15 = st_12 in
  (((poly1305_encode_block (b_11)) +% (acc_13)) *% (r_14), r_14, k_15)

let poly1305_update_blocks (m_16: byte_seq) (st_17: poly_state) : poly_state =
  let st_18 = st_17 in
  let nblocks_19 = (seq_len (m_16)) / (blocksize) in
  let st_18 =
    foldi (usize 0)
      (nblocks_19)
      (fun i_20 st_18 ->
...
let poly1305_update_last
      (pad_len_22: uint_size{(**) pad_len_22 <= 16})
      (b_23: sub_block{(**) Seq.length b_23 = pad_len_22})
      (st_24: poly_state)
    :
    poly_state =
  let st_25 = st_24 in
...
let poly1305_update (m_29: byte_seq) (st_30: poly_state) : poly_state =
...
let poly1305_finish (st_33: poly_state) : tag =
  let acc_34, _, k_35 = st_33 in
  let n_36 =
    uint128_from_le_bytes (array_from_slice (secret (pub_u8 0x0)) (16) (k_35) (usize 16) (usize 16))
  in
  let aby_37 = nat_to_byte_seq_le (0x03fffffffffffffffffffffffffffffffb) (acc_34) in
  let a_38 =
...
let poly1305 (m_39: byte_seq) (key_40: poly_key) : tag =
