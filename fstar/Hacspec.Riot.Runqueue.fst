...
(**) let rq_v (rq: runqueue_id) = let RunqueueId_runqueue_id rq = rq in v rq
(**) let tid_v (tid: thread_id) = let ThreadId_thread_id tid = tid in v tid

let clist_is_empty (x_4: clist) (rq_5: runqueue_id{(**) rq_v rq_5 < n_queues }) : bool =
...
let clist_push (x_9: clist) (n_10: thread_id) (rq_11: runqueue_id) : clist =
  ...
  let tail_14, next_idxs_15 =
    if (array_index (next_idxs_15) (cast U32 PUB (n_13))) = (sentinel)
    then
      let tail_14, next_idxs_15 =
        if (array_index (tail_14) (cast U32 PUB (rq_12))) = (sentinel)
        then
          let tail_14 = array_upd tail_14 (cast U32 PUB (rq_12)) (n_13) in
          let next_idxs_15 = array_upd next_idxs_15 (cast U32 PUB (n_13)) (n_13) in
          ...
          let next_idxs_15 =
            array_upd next_idxs_15
              ...
              (array_index (next_idxs_15)
                  (cast U32 PUB (array_index (tail_14) (cast U32 PUB (rq_12)))))
          in
          let next_idxs_15 =
            array_upd next_idxs_15
              (cast U32 PUB (array_index (tail_14) (cast U32 PUB (rq_12))))
...
let clist_pop_head (x_16: clist) (rq_17: runqueue_id) : (clist & (option pub_uint8)) =
  ...
  let tail_19, next_idxs_20, out_21 =
    if (array_index (tail_19) (rq_18)) = (sentinel)
    then (tail_19, next_idxs_20, out_21)
    else
      let head_22 =
        array_index (next_idxs_20) (cast U32 PUB (array_index (tail_19) (cast U32 PUB (rq_18))))
      in
      let tail_19, next_idxs_20 =
        if (head_22) = (array_index (tail_19) (cast U32 PUB (rq_18)))
        then
          let tail_19 = array_upd tail_19 (cast U32 PUB (rq_18)) (sentinel) in
          (tail_19, next_idxs_20)
        else
          let next_idxs_20 =
            array_upd next_idxs_20
              (cast U32 PUB (array_index (tail_19) (cast U32 PUB (rq_18))))
              (array_index (next_idxs_20) (cast U32 PUB (head_22)))
          in
          (tail_19, next_idxs_20)
      in
      let next_idxs_20 = array_upd next_idxs_20 (head_22) (sentinel) in
...
let clist_peek_head (x_23: clist) (rq_24: runqueue_id) : (option pub_uint8) =
  ...
  if ((array_index (tail_26) (cast U32 PUB (rq_25))) = (sentinel))
  then (None)
  else
    (Some
      ((array_index (next_idxs_27) (cast U32 PUB (array_index (tail_26) (cast U32 PUB (rq_25)))))))
...
let clist_advance (x_28: clist) (rq_29: runqueue_id) : clist =
  ...
  let tail_31 =
    if (array_index (tail_31) (cast U32 PUB (rq_30))) <> (sentinel)
    then
      let tail_31 =
        array_upd tail_31
          (cast U32 PUB (rq_30))
          (array_index (next_idxs_32) (cast U32 PUB (array_index (tail_31) (cast U32 PUB (rq_30)))))
      in
      (tail_31)
    else (tail_31)
  in
...
let del (y_39: run_queue) (n_40: thread_id) (rq_41: runqueue_id) : run_queue =
  ...
  let bitcache_43 =
    ...
    then
      let bitcache_43 =
        (bitcache_43) &. (not ((pub_u32 0x1) `shift_left` (cast U32 PUB (rq_u8_42))))
      in
      (bitcache_43)
    else (bitcache_43)
  in
