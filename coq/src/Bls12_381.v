...
Definition fp12inv (n_111 : fp12) : fp12 :=
  let '(n1_112, n2_113) :=
    n_111 in 
  let gamma_114 : (fp2 × fp2 × fp2) :=
    (fp2zero , fp2fromfp (nat_mod_one ), fp2zero ) in 
  let t1_115 : (fp2 × fp2 × fp2) :=
    fp6mul (n1_112) (n1_112) in 
  let t2_116 : (fp2 × fp2 × fp2) :=
    fp6mul (n2_113) (n2_113) in 
  let t1_117 : (fp2 × fp2 × fp2) :=
    fp6sub (t1_115) (fp6mul (gamma_114) (t2_116)) in 
  let t2_118 : (fp2 × fp2 × fp2) :=
    fp6inv (t1_117) in 
  let x_119 : (fp2 × fp2 × fp2) :=
    fp6mul (n1_112) (t2_118) in 
  let y_120 : (fp2 × fp2 × fp2) :=
...
Definition g1double_a (p_139 : g1) : g1 :=
  ...
  let xovery_143 : fp :=
    ((nat_mod_from_literal (
          ...
          @repr WORDSIZE128 3)) *% (x12_142)) *% (nat_mod_inv ((
...
Definition g2add_a (p_166 : g2) (q_167 : g2) : g2 :=
  ...
  let '(x2_170, y2_171, _) :=
    q_167 in 
  let x_diff_172 : (fp × fp) :=
    fp2sub (x2_170) (x1_168) in 
  let y_diff_173 : (fp × fp) :=
    fp2sub (y2_171) (y1_169) in 
  let xovery_174 : (fp × fp) :=
...
Definition g2double_a (p_181 : g2) : g2 :=
  ...
  let t1_185 : (fp × fp) :=
    fp2mul (fp2fromfp (nat_mod_from_literal (
          0x1a0111ea397fe69a4b1ba7b6434bacd764774b84f38512bf6730d2a0f6b0f6241eabfffeb153ffffb9feffffffffaaab) (
          @repr WORDSIZE128 3))) (x12_184) in 
  let t2_186 : (fp × fp) :=
    fp2inv (fp2mul (fp2fromfp (nat_mod_two )) (y1_183)) in 
  let xovery_187 : (fp × fp) :=
...
Definition twist (p_214 : g1) : (fp12 × fp12) :=
  let '(p0_215, p1_216, _) :=
    p_214 in 
  let x_217 : ((fp2 × fp2 × fp2) × fp6) :=
    ((fp2zero , fp2fromfp (p0_215), fp2zero ), fp6zero ) in 
  let y_218 : (fp6 × (fp2 × fp2 × fp2)) :=
...
Definition line_double_p (r_219 : g2) (p_220 : g1) : fp12 :=
  ...
  let a_223 : (fp × fp) :=
    fp2mul (fp2fromfp (nat_mod_from_literal (
          0x1a0111ea397fe69a4b1ba7b6434bacd764774b84f38512bf6730d2a0f6b0f6241eabfffeb153ffffb9feffffffffaaab) (
          @repr WORDSIZE128 3))) (fp2mul (r0_221) (r0_221)) in 
  let a_224 : (fp × fp) :=
...
Definition frobenius (f_243 : fp12) : fp12 :=
  ...
  let c1_256 : array_fp :=
    array_from_list uint64 (let l :=
        [
          secret (@repr WORDSIZE64 10162220747404304312);
          secret (@repr WORDSIZE64 17761815663483519293);
          secret (@repr WORDSIZE64 8873291758750579140);
          secret (@repr WORDSIZE64 1141103941765652303);
          secret (@repr WORDSIZE64 13993175198059990303);
          secret (@repr WORDSIZE64 1802798568193066599)
        ] in  l) in 
  ...
  let c2_259 : array_fp :=
    array_from_list uint64 (let l :=
        [
          secret (@repr WORDSIZE64 3240210268673559283);
          secret (@repr WORDSIZE64 2895069921743240898);
          secret (@repr WORDSIZE64 17009126888523054175);
          secret (@repr WORDSIZE64 6098234018649060207);
          secret (@repr WORDSIZE64 9865672654120263608);
          secret (@repr WORDSIZE64 71000049454473266)
        ] in  l) in 
  let c2_260 : seq uint8 :=
    array_to_le_bytes (c2_259) in 
  let c2_261 : fp :=
    nat_mod_from_byte_seq_le (c2_260) in 
  let gamma11_262 : (fp × fp) :=
    (c1_258, c2_261) in 
  let gamma12_263 : (fp × fp) :=
    fp2mul (gamma11_262) (gamma11_262) in 
  let gamma13_264 : (fp × fp) :=
    fp2mul (gamma12_263) (gamma11_262) in 
  let gamma14_265 : (fp × fp) :=
    fp2mul (gamma13_264) (gamma11_262) in 
  let gamma15_266 : (fp × fp) :=
...
Definition final_exponentiation (f_272 : fp12) : fp12 :=
  ...
  let finv_274 : (fp6 × fp6) :=
    fp12inv (f_272) in 
  let fp6_1_275 : (fp6 × fp6) :=
  ...
  let u_278 : scalar :=
    nat_mod_from_literal (
      ...
      @repr WORDSIZE128 15132376222941642752) in 
  let t0_279 : (fp6 × fp6) :=
