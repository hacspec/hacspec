    ...
    foldi (usize 0) (count_15) (fun i_19 output_18 =>
      ...
      let u_i_22 : fp :=
        nat_mod_from_byte_seq_be (seq_slice (nat_mod_to_byte_seq_be (
              nat_mod_from_byte_seq_be (tv_21) : fp_hash)) (usize 16) (
            usize 48)) : fp in 
      let output_18 :=
...
Definition fp_is_square (x_24 : fp) : bool :=
  let c1_25 : fp :=
    nat_mod_from_byte_seq_be (array_to_be_bytes (p_1_2)) : fp in 
  let tv_26 : fp :=
...
Definition g1_map_to_curve_svdw (u_30 : fp) : g1 :=
  ...
  let tv4_37 : fp :=
    fp_sqrt (((nat_mod_zero ) -% (gz_32)) *% (((nat_mod_from_literal (_) (
              @repr WORDSIZE128 3) : fp) *% (z_31)) *% (z_31))) in 
  let '(tv4_37) :=
  ...
  let tv6_39 : fp :=
    (((nat_mod_zero ) -% (nat_mod_from_literal (_) (
            @repr WORDSIZE128 4) : fp)) *% (gz_32)) *% (nat_mod_inv (((
            nat_mod_from_literal (_) (@repr WORDSIZE128 3) : fp) *% (z_31)) *% (
          z_31))) in 
  let x1_40 : fp :=
...
Definition g1_clear_cofactor (x_45 : g1) : g1 :=
  let h_eff_46 : scalar :=
    nat_mod_from_literal (_) (
      @repr WORDSIZE128 15132376222941642753) : scalar in 
  g1mul (h_eff_46) (x_45).
...
Definition fp2_hash_to_field
  ...
  let output_64 :=
    foldi (usize 0) (count_61) (fun i_65 output_64 =>
      ...
      let e_1_68 : fp :=
        nat_mod_from_byte_seq_be (seq_slice (nat_mod_to_byte_seq_be (
              nat_mod_from_byte_seq_be (tv_67) : fp_hash)) (usize 16) (
            usize 48)) : fp in 
      ...
      let e_2_71 : fp :=
        nat_mod_from_byte_seq_be (seq_slice (nat_mod_to_byte_seq_be (
              nat_mod_from_byte_seq_be (tv_70) : fp_hash)) (usize 16) (
            usize 48)) : fp in 
      let output_64 :=
...
Definition fp2_sqrt (a_91 : fp2) : fp2 :=
  let c1_92 : fp :=
    nat_mod_from_byte_seq_be (array_to_be_bytes (p_3_4)) : fp in 
  let c2_93 : fp :=
    nat_mod_from_byte_seq_be (array_to_be_bytes (p_1_2)) : fp in 
  let a1_94 : (fp × fp) :=
...
Definition g2_curve_func (x_99 : fp2) : fp2 :=
  fp2add (fp2mul (x_99) (fp2mul (x_99) (x_99))) ((
      nat_mod_from_literal (_) (@repr WORDSIZE128 4) : fp,
      nat_mod_from_literal (_) (@repr WORDSIZE128 4) : fp
    )).
...
Definition g2_map_to_curve_svdw (u_100 : fp2) : g2 :=
  ...
  let tv4_107 : (fp × fp) :=
    fp2_sqrt (fp2mul (fp2neg (gz_102)) (fp2mul (fp2fromfp (
            nat_mod_from_literal (_) (@repr WORDSIZE128 3) : fp)) (fp2mul (
            z_101) (z_101)))) in 
  let '(tv4_107) :=
  ...
  let tv6_109 : (fp × fp) :=
    fp2mul (fp2mul (fp2neg (fp2fromfp (nat_mod_from_literal (_) (
              @repr WORDSIZE128 4) : fp))) (gz_102)) (fp2inv (fp2mul (
          fp2fromfp (nat_mod_from_literal (_) (@repr WORDSIZE128 3) : fp)) (
...
Definition g2_clear_cofactor (p_124 : g2) : g2 :=
  let c1_125 : scalar :=
    nat_mod_from_literal (_) (
      @repr WORDSIZE128 15132376222941642752) : scalar in 
...
Definition g1_simple_swu_iso (u_150 : fp) : (fp × fp) :=
  let z_151 : fp :=
    nat_mod_from_literal (_) (@repr WORDSIZE128 11) : fp in 
  let a_152 : fp :=
    nat_mod_from_byte_seq_be (array_to_be_bytes (g1_iso_a)) : fp in 
  let b_153 : fp :=
    nat_mod_from_byte_seq_be (array_to_be_bytes (g1_iso_b)) : fp in 
  let tv1_154 : fp :=
...
Definition g2_simple_swu_iso (u_198 : fp2) : (fp2 × fp2) :=
  ...
  let a_200 : (fp × fp) :=
    (nat_mod_zero , nat_mod_from_literal (_) (@repr WORDSIZE128 240) : fp) in 
  let b_201 : (fp × fp) :=
    (
      nat_mod_from_literal (_) (@repr WORDSIZE128 1012) : fp,
      nat_mod_from_literal (_) (@repr WORDSIZE128 1012) : fp
    ) in 
  let tv1_202 : (fp × fp) :=
...
Definition g2_isogeny_map (x_209 : fp2) (y_210 : fp2) : g2 :=
  ...
  let xnum_k_211 :=
    seq_upd xnum_k_211 (usize 0) ((
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xnum_k_0)) : fp,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xnum_k_0)) : fp
      )) in 
  let xnum_k_211 :=
    seq_upd xnum_k_211 (usize 1) ((
        nat_mod_zero ,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xnum_k_1_i)) : fp
      )) in 
  let xnum_k_211 :=
    seq_upd xnum_k_211 (usize 2) ((
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xnum_k_2_r)) : fp,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xnum_k_2_i)) : fp
      )) in 
  let xnum_k_211 :=
    seq_upd xnum_k_211 (usize 3) ((
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xnum_k_3_r)) : fp,
        nat_mod_zero 
      )) in 
  ...
  let xden_k_212 :=
    seq_upd xden_k_212 (usize 0) ((
        nat_mod_zero ,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xden_k_0_i)) : fp
      )) in 
  let xden_k_212 :=
    seq_upd xden_k_212 (usize 1) ((
        nat_mod_from_literal (_) (@repr WORDSIZE128 12) : fp,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_xden_k_1_i)) : fp
      )) in 
  let ynum_k_213 : seq (fp × fp) :=
    seq_new_ (default) (usize 4) in 
  let ynum_k_213 :=
    seq_upd ynum_k_213 (usize 0) ((
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_ynum_k_0)) : fp,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_ynum_k_0)) : fp
      )) in 
  let ynum_k_213 :=
    seq_upd ynum_k_213 (usize 1) ((
        nat_mod_zero ,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_ynum_k_1_i)) : fp
      )) in 
  let ynum_k_213 :=
    seq_upd ynum_k_213 (usize 2) ((
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_ynum_k_2_r)) : fp,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_ynum_k_2_i)) : fp
      )) in 
  let ynum_k_213 :=
    seq_upd ynum_k_213 (usize 3) ((
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_ynum_k_3_r)) : fp,
        nat_mod_zero 
      )) in 
  ...
  let yden_k_214 :=
    seq_upd yden_k_214 (usize 0) ((
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_yden_k_0)) : fp,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_yden_k_0)) : fp
      )) in 
  let yden_k_214 :=
    seq_upd yden_k_214 (usize 1) ((
        nat_mod_zero ,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_yden_k_1_i)) : fp
      )) in 
  let yden_k_214 :=
    seq_upd yden_k_214 (usize 2) ((
        nat_mod_from_literal (_) (@repr WORDSIZE128 18) : fp,
        nat_mod_from_byte_seq_be (array_to_be_bytes (g2_yden_k_2_i)) : fp
      )) in 
  let xnum_215 : (fp × fp) :=
    fp2zero  in 
  let xx_216 : (fp × fp) :=
