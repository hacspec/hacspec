...
Class WORDSIZE := {
  wordsize : nat;
  wordsize_not_zero: wordsize <> 0%nat;
}.
...
(* Hypothesis wordsize_not_zero: wordsize <> 0%nat. *)
...
Section Int32.
  Let wordsize32 := 32%nat.
  Let wordsize_not_zero: wordsize32 <> 0%nat.
  Proof. unfold wordsize32; congruence. Qed.
  Instance WORDSIZE32 : WORDSIZE := {
    wordsize := wordsize32;
...
Section Int8.
  Let wordsize8 := 8%nat.
  Let wordsize_not_zero: wordsize8 <> 0%nat.
  Proof. unfold wordsize8; congruence. Qed.
  Instance WORDSIZE8 : WORDSIZE := {
    wordsize := wordsize8;
    wordsize_not_zero := wordsize_not_zero
  }.
...
Section Int16.
  Let wordsize16 := 16%nat.
  Let wordsize_not_zero: wordsize16 <> 0%nat.
  Proof. unfold wordsize16; congruence. Qed.
  Instance WORDSIZE16 : WORDSIZE := {
    wordsize := wordsize16;
    wordsize_not_zero := wordsize_not_zero
  }.
...
Section Int64.
  Let wordsize64 := 64%nat.
  Let wordsize_not_zero: wordsize64 <> 0%nat.
  Proof. unfold wordsize64; congruence. Qed.
  Instance WORDSIZE64 : WORDSIZE := {
    wordsize := wordsize64;
    wordsize_not_zero := wordsize_not_zero
  }.
...
Section Int128.
  Let wordsize128 := 128%nat.
  Let wordsize_not_zero: wordsize128 <> 0%nat.
  Proof. unfold wordsize128; congruence. Qed.
  Instance WORDSIZE128 : WORDSIZE := {
    wordsize := wordsize128;
    wordsize_not_zero := wordsize_not_zero
  }.
...
Definition shl' (x: int64) (y: int32): int64 :=
  repr (Z.shiftl (unsigned x) (@unsigned WORDSIZE32 y)).
Definition shru' (x: int64) (y: int32): int64 :=
  repr (Z.shiftr (unsigned x) (@unsigned WORDSIZE32 y)).
Definition shr' (x: int64) (y: int32): int64 :=
  repr (Z.shiftr (signed x) (@unsigned WORDSIZE32 y)).
Definition rol' (x: int64) (y: int32): int64 :=
  rol x (repr (@unsigned WORDSIZE32 y)).
Definition shrx' (x: int64) (y: int32): int64 :=
  divs x (shl' one y).
Definition shr_carry' (x: int64) (y: int32): int64 :=
  if lt x zero && negb (eq (and x (sub (shl' one y) one)) zero)
  then one else zero.
...
Lemma bits_shl':
  ...
  0 <= i < zwordsize ->
  testbit (shl' x y) i =
  if zlt i (@unsigned WORDSIZE32 y) then false else testbit x (i - @unsigned WORDSIZE32 y).
Proof.
  intros. unfold shl'. rewrite testbit_repr; auto.
  destruct (zlt i (@unsigned WORDSIZE32 y)).
  apply Z.shiftl_spec_low. auto.
  apply Z.shiftl_spec_high. lia. lia.
Qed.
...
Lemma bits_shru':
  ...
  0 <= i < zwordsize ->
  testbit (shru' x y) i =
  if zlt (i + @unsigned WORDSIZE32 y) zwordsize then testbit x (i + @unsigned WORDSIZE32 y) else false.
Proof.
  intros. unfold shru'. rewrite testbit_repr; auto.
  rewrite Z.shiftr_spec. fold (testbit x (i + @unsigned WORDSIZE32 y)).
  destruct (zlt (i + @unsigned WORDSIZE32 y) zwordsize).
  auto.
  apply bits_above; auto.
  lia.
Qed.
...
Lemma bits_shr':
  ...
  0 <= i < zwordsize ->
  testbit (shr' x y) i =
  testbit x (if zlt (i + @unsigned WORDSIZE32 y) zwordsize then i + @unsigned WORDSIZE32 y else zwordsize - 1).
Proof.
  intros. unfold shr'. rewrite testbit_repr; auto.
  rewrite Z.shiftr_spec. apply bits_signed.
...
Lemma shl'_mul_two_p:
  forall x y,
  shl' x y = mul x (repr (two_p (@unsigned WORDSIZE32 y))).
Proof.
  intros. unfold shl', mul. apply eqm_samerepr.
...
Proof.
  intros. unfold shl'. rewrite (@unsigned_zero WORDSIZE32). unfold Z.shiftl.
  apply repr_unsigned.
Qed.

Theorem shru'_zero :
  forall x, shru' x (@zero WORDSIZE32) = x.
Proof.
  intros. unfold shru'. rewrite (@unsigned_zero WORDSIZE32). unfold Z.shiftr.
  apply repr_unsigned.
Qed.

Theorem shr'_zero :
  forall x, shr' x (@zero WORDSIZE32) = x.
Proof.
  intros. unfold shr'. rewrite (@unsigned_zero WORDSIZE32). unfold Z.shiftr.
  apply repr_signed.
Qed.

Theorem shrx'_zero:
  forall x, shrx' x (@zero WORDSIZE32) = x.
Proof.
  intros. change (shrx' x (@zero WORDSIZE32)) with (shrx x zero). apply shrx_zero. compute; auto.
Qed.

Theorem shrx'_carry:
  forall x y,
  @ltu WORDSIZE32 y (@repr WORDSIZE32 63) = true ->
  shrx' x y = add (shr' x y) (shr_carry' x y).
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H. change (@unsigned WORDSIZE32 (@repr WORDSIZE32 63)) with 63 in H.
  set (y1 := repr (@unsigned WORDSIZE32 y)).
  assert (U: unsigned y1 = @unsigned WORDSIZE32 y).
...
Theorem shrx'_shr_2:
  forall x y,
  @ltu WORDSIZE32 y (@repr WORDSIZE32 63) = true ->
  shrx' x y = shr' (add x (shru' (shr' x (@repr WORDSIZE32 63)) (@sub WORDSIZE32 (@repr WORDSIZE32 64) y))) y.
Proof.
  intros.
  set (z := repr (@unsigned WORDSIZE32 y)).
  apply (@ltu_inv WORDSIZE32) in H. change (@unsigned WORDSIZE32 (@repr WORDSIZE32 63)) with 63 in H.
  assert (N1: 63 < max_unsigned) by reflexivity.
  assert (N2: 63 < @max_unsigned WORDSIZE32) by reflexivity.
  assert (A: unsigned z = @unsigned WORDSIZE32 y).
  { unfold z; apply unsigned_repr; lia. }
  assert (B: unsigned (sub (repr 64) z) = @unsigned WORDSIZE32 (@sub WORDSIZE32 (@repr WORDSIZE32 64) y)).
  { unfold z. unfold sub.
    change (unsigned (repr 64)) with 64.
    change (@unsigned WORDSIZE32 (@repr WORDSIZE32 64)) with 64.
    rewrite (unsigned_repr (@unsigned WORDSIZE32 y)) by lia.
    rewrite unsigned_repr by lia. rewrite (@unsigned_repr WORDSIZE32) by lia.
    reflexivity.
     }
  unfold shrx', shr', shru', shl'.
  rewrite <- A.
  change (@unsigned WORDSIZE32 (@repr WORDSIZE32 63)) with (unsigned (repr 63)).
  rewrite <- B.
  apply (@shrx_shr_2 WORDSIZE64).
  unfold ltu. apply zlt_true. change (unsigned z < 63). rewrite A; lia.  
Qed.
(* 
Remark int_ltu_2_inv:
  forall y z,
  @ltu WORDSIZE32 y iwordsize' = true ->
  @ltu WORDSIZE32 z iwordsize' = true ->
  @unsigned WORDSIZE32 (Int.add y z) <= @unsigned WORDSIZE32 iwordsize' ->
  let y' := repr (@unsigned WORDSIZE32 y) in
  let z' := repr (@unsigned WORDSIZE32 z) in
     @unsigned WORDSIZE32 y = unsigned y'
  /\ @unsigned WORDSIZE32 z = unsigned z'
  /\ ltu y' iwordsize = true
  /\ ltu z' iwordsize = true
  /\ @unsigned WORDSIZE32 (Int.add y z) = unsigned (add y' z')
  /\ add y' z' = repr (@unsigned WORDSIZE32 (Int.add y z)).
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H. apply (@ltu_inv WORDSIZE32) in H0.
  change (@unsigned WORDSIZE32 iwordsize') with 64 in *.
  assert (128 < max_unsigned) by reflexivity.
  assert (128 < @max_unsigned WORDSIZE32) by reflexivity.
  assert (Y: unsigned y' = @unsigned WORDSIZE32 y) by (apply unsigned_repr; lia).
  assert (Z: unsigned z' = @unsigned WORDSIZE32 z) by (apply unsigned_repr; lia).
  assert (P: @unsigned WORDSIZE32 (Int.add y z) = unsigned (add y' z')).
  { unfold Int.add. rewrite @unsigned WORDSIZE32_repr by lia.
...
Theorem or_ror':
  ...
  ror x (repr (@unsigned WORDSIZE32 z)) = or (shl' x y) (shru' x z).
Proof.
  intros. destruct (int_ltu_2_inv y z) as (A & B & C & D & E & F); auto. rewrite H1; lia.
  replace (shl' x y) with (shl x (repr (@unsigned WORDSIZE32 y))).
  replace (shru' x z) with (shru x (repr (@unsigned WORDSIZE32 z))).
...
Theorem shl'_shl':
  ...
  @ltu WORDSIZE32 y iwordsize' = true ->
  @ltu WORDSIZE32 z iwordsize' = true ->
  @ltu WORDSIZE32 (Int.add y z) iwordsize' = true ->
  shl' (shl' x y) z = shl' x (Int.add y z).
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H1.
  destruct (int_ltu_2_inv y z) as (A & B & C & D & E & F); auto. lia.
  set (y' := repr (@unsigned WORDSIZE32 y)) in *.
  set (z' := repr (@unsigned WORDSIZE32 z)) in *.
...
Theorem shru'_shl':
  forall x y z, @ltu WORDSIZE32 y iwordsize' = true -> @ltu WORDSIZE32 z iwordsize' = true ->
  shru' (shl' x y) z =
  if @ltu WORDSIZE32 z y then shl' (zero_ext (zwordsize - @unsigned WORDSIZE32 y) x) (@sub WORDSIZE32 y z)
                 else zero_ext (zwordsize - @unsigned WORDSIZE32 z) (shru' x (@sub WORDSIZE32 z y)).
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H; apply (@ltu_inv WORDSIZE32) in H0.
  change (@unsigned WORDSIZE32 iwordsize') with zwordsize in *.
  unfold @ltu WORDSIZE32. set (Y := @unsigned WORDSIZE32 y) in *; set (Z := @unsigned WORDSIZE32 z) in *.
  apply same_bits_eq; intros. rewrite bits_shru' by auto. fold Z.
  destruct (zlt Z Y).
- assert (A: @unsigned WORDSIZE32 (@sub WORDSIZE32 y z) = Y - Z).
  { apply @unsigned WORDSIZE32_repr. assert (zwordsize < @max_unsigned WORDSIZE32) by reflexivity. lia. }
  symmetry; rewrite bits_shl', A by lia.
  destruct (zlt (i + Z) zwordsize).
+ rewrite bits_shl' by lia. fold Y.
  destruct (zlt i (Y - Z)); [rewrite zlt_true by lia|rewrite zlt_false by lia]; auto.
  rewrite bits_zero_ext by lia. rewrite zlt_true by lia. f_equal; lia.
+ rewrite bits_zero_ext by lia. rewrite ! zlt_false by lia. auto.
- assert (A: @unsigned WORDSIZE32 (@sub WORDSIZE32 z y) = Z - Y).
  { apply @unsigned WORDSIZE32_repr. assert (zwordsize < @max_unsigned WORDSIZE32) by reflexivity. lia. }
  rewrite bits_zero_ext, bits_shru', A by lia.
  destruct (zlt (i + Z) zwordsize); [rewrite zlt_true by lia|rewrite zlt_false by lia]; auto.
  rewrite bits_shl' by lia. fold Y.
...
Theorem shr'_shl':
  forall x y z, @ltu WORDSIZE32 y iwordsize' = true -> @ltu WORDSIZE32 z iwordsize' = true ->
  shr' (shl' x y) z =
  if @ltu WORDSIZE32 z y then shl' (sign_ext (zwordsize - @unsigned WORDSIZE32 y) x) (@sub WORDSIZE32 y z)
                 else sign_ext (zwordsize - @unsigned WORDSIZE32 z) (shr' x (@sub WORDSIZE32 z y)).
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H; apply (@ltu_inv WORDSIZE32) in H0.
  change (@unsigned WORDSIZE32 iwordsize') with zwordsize in *.
  unfold @ltu WORDSIZE32. set (Y := @unsigned WORDSIZE32 y) in *; set (Z := @unsigned WORDSIZE32 z) in *.
...
Lemma shl'_zero_ext:
  forall n m x, 0 <= n ->
  shl' (zero_ext n x) m = zero_ext (n + @unsigned WORDSIZE32 m) (shl' x m).
Proof.
  intros. apply same_bits_eq; intros.
  rewrite bits_zero_ext, ! bits_shl' by lia.
  destruct (zlt i (@unsigned WORDSIZE32 m)).
- rewrite zlt_true by lia; auto.
- rewrite bits_zero_ext by lia.
  destruct (zlt (i - @unsigned WORDSIZE32 m) n); [rewrite zlt_true by lia|rewrite zlt_false by lia]; auto.
Qed.

Lemma shl'_sign_ext:
  forall n m x, 0 < n ->
  shl' (sign_ext n x) m = sign_ext (n + @unsigned WORDSIZE32 m) (shl' x m).
Proof.
  intros. generalize (@unsigned_range WORDSIZE32 m); intros.
  apply same_bits_eq; intros.
  rewrite bits_sign_ext, ! bits_shl' by lia.
  destruct (zlt i (n + @unsigned WORDSIZE32 m)).
- rewrite bits_shl' by auto. destruct (zlt i (@unsigned WORDSIZE32 m)); auto.
  rewrite bits_sign_ext by lia. f_equal. apply zlt_true. lia.
- rewrite zlt_false by lia. rewrite bits_shl' by lia. rewrite zlt_false by lia.
  rewrite bits_sign_ext by lia. f_equal. rewrite zlt_false by lia. lia.
Qed.
...
Lemma shru'_zero_ext:
  forall n m x, 0 <= n ->
  shru' (zero_ext (n + @unsigned WORDSIZE32 m) x) m = zero_ext n (shru' x m).
Proof.
  intros. generalize (@unsigned_range WORDSIZE32 m); intros.
  bit_solve; [|lia]. rewrite bits_shru', bits_zero_ext, bits_shru' by lia.
  destruct (zlt (i + @unsigned WORDSIZE32 m) zwordsize).
* destruct (zlt i n); [rewrite zlt_true by lia|rewrite zlt_false by lia]; auto.
* destruct (zlt i n); auto.
Qed.

Lemma shru'_zero_ext_0:
  forall n m x, n <= @unsigned WORDSIZE32 m ->
  shru' (zero_ext n x) m = zero.
Proof.
  intros. generalize (@unsigned_range WORDSIZE32 m); intros.
  bit_solve. rewrite bits_shru', bits_zero_ext by lia.
  destruct (zlt (i + @unsigned WORDSIZE32 m) zwordsize); auto.
  apply zlt_false. lia.
Qed.

Lemma shr'_sign_ext:
  forall n m x, 0 < n -> n + @unsigned WORDSIZE32 m < zwordsize ->
  shr' (sign_ext (n + @unsigned WORDSIZE32 m) x) m = sign_ext n (shr' x m).
Proof.
  intros. generalize (@unsigned_range WORDSIZE32 m); intros.
  apply same_bits_eq; intros.
  rewrite bits_sign_ext, bits_shr' by auto.
  rewrite bits_sign_ext, bits_shr'.
- f_equal.
  destruct (zlt i n), (zlt (i + @unsigned WORDSIZE32 m) zwordsize).
+ apply zlt_true; lia.
+ apply zlt_true; lia.
+ rewrite zlt_false by lia. rewrite zlt_true by lia. lia.
+ rewrite zlt_false by lia. rewrite zlt_true by lia. lia.
- destruct (zlt i n); lia.
- destruct (zlt (i + @unsigned WORDSIZE32 m) zwordsize); lia.
Qed.

Lemma zero_ext_shru'_min:
  forall s x n, @ltu WORDSIZE32 n iwordsize' = true ->
  zero_ext s (shru' x n) = zero_ext (Z.min s (zwordsize - @unsigned WORDSIZE32 n)) (shru' x n).
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H. change (@unsigned WORDSIZE32 iwordsize') with zwordsize in H.
  apply Z.min_case_strong; intros; auto.
  bit_solve; try lia. rewrite ! bits_shru' by lia. 
  destruct (zlt i (zwordsize - @unsigned WORDSIZE32 n)).
  rewrite zlt_true by lia. auto.
  destruct (zlt i s); auto. rewrite zlt_false by lia; auto.
Qed.
...
Lemma sign_ext_shr'_min:
  forall s x n, @ltu WORDSIZE32 n iwordsize' = true ->
  sign_ext s (shr' x n) = sign_ext (Z.min s (zwordsize - @unsigned WORDSIZE32 n)) (shr' x n).
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H. change (@unsigned WORDSIZE32 iwordsize') with zwordsize in H.
  rewrite Z.min_comm. 
  destruct (Z.min_spec (zwordsize - @unsigned WORDSIZE32 n) s) as [[A B] | [A B]]; rewrite B; auto.
  apply same_bits_eq; intros. rewrite ! bits_sign_ext by auto.
  destruct (zlt i (zwordsize - @unsigned WORDSIZE32 n)). 
  rewrite zlt_true by lia. auto.
  assert (C: testbit (shr' x n) (zwordsize - @unsigned WORDSIZE32 n - 1) = testbit x (zwordsize - 1)).
  { rewrite bits_shr' by lia. rewrite zlt_true by lia. f_equal; lia. }
  rewrite C. destruct (zlt i s); rewrite bits_shr' by lia.
  rewrite zlt_false by lia. auto.
  rewrite zlt_false by lia. auto.
...
Lemma shl'_zero_ext_min:
  forall s x n, @ltu WORDSIZE32 n iwordsize' = true ->
  shl' (zero_ext s x) n = shl' (zero_ext (Z.min s (zwordsize - @unsigned WORDSIZE32 n)) x) n.
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H. change (@unsigned WORDSIZE32 iwordsize') with zwordsize in H.
  apply Z.min_case_strong; intros; auto.
  apply same_bits_eq; intros. rewrite ! bits_shl' by auto.
  destruct (zlt i (@unsigned WORDSIZE32 n)); auto.
  rewrite ! bits_zero_ext by lia.
  destruct (zlt (i - @unsigned WORDSIZE32 n) s).
  rewrite zlt_true by lia; auto.
  rewrite zlt_false by lia; auto.
Qed.
...
Lemma shl'_sign_ext_min:
  forall s x n, @ltu WORDSIZE32 n iwordsize' = true ->
  shl' (sign_ext s x) n = shl' (sign_ext (Z.min s (zwordsize - @unsigned WORDSIZE32 n)) x) n.
Proof.
  intros. apply (@ltu_inv WORDSIZE32) in H. change (@unsigned WORDSIZE32 iwordsize') with zwordsize in H.
  rewrite Z.min_comm. 
  destruct (Z.min_spec (zwordsize - @unsigned WORDSIZE32 n) s) as [[A B] | [A B]]; rewrite B; auto.
...
Definition one_bits' (x: int64) : list int32 :=
  List.map @repr WORDSIZE32 (Z_one_bits wordsize (unsigned x) 0).

Definition is_power2' (x: int64) : option int32 :=
  match Z_one_bits wordsize (unsigned x) 0 with
  | i :: nil => Some (@repr WORDSIZE32 i)
  | _ => None
  end.

Theorem one_bits'_range:
  forall x i, In i (one_bits' x) -> @ltu WORDSIZE32 i iwordsize' = true.
Proof.
  intros.
  destruct (list_in_map_inv _ _ _ H) as [i0 [EQ IN]].
  exploit Z_one_bits_range; eauto. fold zwordsize. intros R.
  unfold @ltu WORDSIZE32. rewrite EQ. rewrite @unsigned WORDSIZE32_repr.
  change (@unsigned WORDSIZE32 iwordsize') with zwordsize. apply zlt_true. lia.
  assert (zwordsize < @max_unsigned WORDSIZE32) by reflexivity. lia.
Qed.

Fixpoint int_of_one_bits' (l: list int32) : int64 :=
  match l with
  | nil => zero
  | a :: b => add (shl' one a) (int_of_one_bits' b)
  end.
...
Proof.
  assert (REC: forall l,
           ...
           int_of_one_bits' (List.map @repr WORDSIZE32 l) = repr (powerserie l)).
  { induction l; simpl; intros.
  - auto.
  - rewrite IHl by eauto. apply eqm_samerepr; apply eqm_add.
  + rewrite shl'_one_two_p. rewrite @unsigned WORDSIZE32_repr. apply eqm_sym; apply eqm_unsigned_repr.
    exploit (H a). auto. assert (zwordsize < @max_unsigned WORDSIZE32) by reflexivity. lia.
...
Lemma is_power2'_rng:
  forall n logn,
  is_power2' n = Some logn ->
  0 <= @unsigned WORDSIZE32 logn < zwordsize.
Proof.
  ...
  rewrite @unsigned WORDSIZE32_repr. auto.
  assert (zwordsize < @max_unsigned WORDSIZE32) by reflexivity.
  lia.
Qed.

Theorem is_power2'_range:
  forall n logn,
  is_power2' n = Some logn -> @ltu WORDSIZE32 logn iwordsize' = true.
Proof.
  intros. unfold @ltu WORDSIZE32. change (@unsigned WORDSIZE32 iwordsize') with zwordsize.
...
Lemma is_power2'_correct:
  forall n logn,
  is_power2' n = Some logn ->
  unsigned n = two_p (@unsigned WORDSIZE32 logn).
Proof.
  unfold is_power2'; intros.
  ...
  rewrite @unsigned WORDSIZE32_repr. rewrite B; simpl. lia.
  assert (0 <= i < zwordsize).
  { apply Z_one_bits_range with (unsigned n). rewrite B; auto with coqlib. }
  assert (zwordsize < @max_unsigned WORDSIZE32) by reflexivity.
  lia.
...
Definition loword (n: int64) : int32 := @repr WORDSIZE32 (unsigned n).

Definition hiword (n: int64) : int32 := @repr WORDSIZE32 (unsigned (shru n (repr Int.zwordsize))).

Definition ofwords (hi lo: int32) : int64 :=
  or (shl (repr (@unsigned WORDSIZE32 hi)) (repr Int.zwordsize)) (repr (@unsigned WORDSIZE32 lo)).
...
Lemma ofwords_add:
  forall lo hi, ofwords hi lo = repr (@unsigned WORDSIZE32 hi * two_p 32 + @unsigned WORDSIZE32 lo).
Proof.
  ...
  rewrite unsigned_repr. generalize (@unsigned_range WORDSIZE32 lo). intros [A B]. exact B.
  assert (@max_unsigned WORDSIZE32 < max_unsigned) by (compute; auto).
  generalize (@unsigned_range WORDSIZE32_2 lo); lia.
Qed.

Lemma ofwords_add':
  forall lo hi, unsigned (ofwords hi lo) = @unsigned WORDSIZE32 hi * two_p 32 + @unsigned WORDSIZE32 lo.
Proof.
  intros. rewrite ofwords_add. apply unsigned_repr.
  generalize (@unsigned_range WORDSIZE32 hi) (@unsigned_range WORDSIZE32 lo).
...
Lemma ofwords_add'':
  forall lo hi, signed (ofwords hi lo) = Int.signed hi * two_p 32 + @unsigned WORDSIZE32 lo.
Proof.
  intros. rewrite ofwords_add.
  replace (repr (@unsigned WORDSIZE32 hi * two_p 32 + @unsigned WORDSIZE32 lo))
     with (repr (Int.signed hi * two_p 32 + @unsigned WORDSIZE32 lo)).
  apply signed_repr.
  generalize (Int.signed_range hi) (@unsigned_range WORDSIZE32 lo).
  change (two_p 32) with Int.modulus.
...
Lemma decompose_shl_1:
  forall xh xl y,
  0 <= @unsigned WORDSIZE32 y < Int.zwordsize ->
  shl' (ofwords xh xl) y =
  ofwords (Int.or (Int.shl xh y) (Int.shru xl (@sub WORDSIZE32 Int.iwordsize y)))
          (Int.shl xl y).
Proof.
  intros.
  assert (@unsigned WORDSIZE32 (@sub WORDSIZE32 Int.iwordsize y) = Int.zwordsize - @unsigned WORDSIZE32 y).
  { unfold @sub WORDSIZE32. rewrite @unsigned WORDSIZE32_repr. auto.
    rewrite @unsigned WORDSIZE32_repr_wordsize. generalize Int.wordsize_max_unsigned; lia. }
  assert (zwordsize = 2 * Int.zwordsize) by reflexivity.
  ...
  destruct (zlt i (@unsigned WORDSIZE32 y)). auto.
  ...
  destruct (zlt (i - @unsigned WORDSIZE32 y) (Int.zwordsize)).
...
Lemma decompose_shl_2:
  forall xh xl y,
  Int.zwordsize <= @unsigned WORDSIZE32 y < zwordsize ->
  shl' (ofwords xh xl) y =
  ofwords (Int.shl xl (@sub WORDSIZE32 y Int.iwordsize)) (@zero WORDSIZE32).
Proof.
  intros.
  assert (zwordsize = 2 * Int.zwordsize) by reflexivity.
  assert (@unsigned WORDSIZE32 (@sub WORDSIZE32 y Int.iwordsize) = @unsigned WORDSIZE32 y - Int.zwordsize).
  { unfold @sub WORDSIZE32. rewrite @unsigned WORDSIZE32_repr. auto.
    rewrite @unsigned WORDSIZE32_repr_wordsize. generalize (@unsigned_range WORDSIZE32_2 y). lia. }
  apply Int64.same_bits_eq; intros.
  rewrite bits_shl' by auto. symmetry. rewrite bits_ofwords by auto.
...
Lemma decompose_shru_1:
  ...
  ofwords (Int.shru xh y)
          (Int.or (Int.shru xl y) (Int.shl xh (@sub WORDSIZE32 Int.iwordsize y))).
Proof.
  intros.
  assert (@unsigned WORDSIZE32 (@sub WORDSIZE32 Int.iwordsize y) = Int.zwordsize - @unsigned WORDSIZE32 y).
  { unfold @sub WORDSIZE32. rewrite @unsigned WORDSIZE32_repr. auto.
...
Lemma decompose_shru_2:
  ...
  Int.zwordsize <= @unsigned WORDSIZE32 y < zwordsize ->
  shru' (ofwords xh xl) y =
  ofwords (@zero WORDSIZE32) (Int.shru xh (@sub WORDSIZE32 y Int.iwordsize)).
Proof.
  intros.
  assert (zwordsize = 2 * Int.zwordsize) by reflexivity.
  assert (@unsigned WORDSIZE32 (@sub WORDSIZE32 y Int.iwordsize) = @unsigned WORDSIZE32 y - Int.zwordsize).
...
Lemma decompose_shr_2:
  ...
  Int.zwordsize <= @unsigned WORDSIZE32 y < zwordsize ->
  shr' (ofwords xh xl) y =
  ofwords (Int.shr xh (@sub WORDSIZE32 Int.iwordsize Int.one))
          (Int.shr xh (@sub WORDSIZE32 y Int.iwordsize)).
Proof.
  intros.
  assert (zwordsize = 2 * Int.zwordsize) by reflexivity.
  assert (@unsigned WORDSIZE32 (@sub WORDSIZE32 y Int.iwordsize) = @unsigned WORDSIZE32 y - Int.zwordsize).
  ...
  rewrite zlt_false by lia. auto.
  rewrite Int.bits_shr by lia.
  change (@unsigned WORDSIZE32 (@sub WORDSIZE32 Int.iwordsize Int.one)) with (Int.zwordsize - 1).
  destruct (zlt (i + @unsigned WORDSIZE32 y) zwordsize);
  rewrite bits_ofwords by lia.
...
Lemma decompose_add:
  ...
  ofwords (Int.add (Int.add xh yh) (Int.add_carry xl yl (@zero WORDSIZE32)))
          (Int.add xl yl).
Proof.
  intros. symmetry. rewrite ofwords_add. rewrite add_unsigned.
  apply eqm_samerepr.
  rewrite ! ofwords_add'. rewrite (@unsigned WORDSIZE32_add_carry xl yl).
  set (cc := Int.add_carry xl yl (@zero WORDSIZE32)).
  set (Xl := @unsigned WORDSIZE32 xl); set (Xh := @unsigned WORDSIZE32 xh);
  set (Yl := @unsigned WORDSIZE32 yl); set (Yh := @unsigned WORDSIZE32 yh).
  change Int.modulus with (two_p 32).
  replace (Xh * two_p 32 + Xl + (Yh * two_p 32 + Yl))
     with ((Xh + Yh) * two_p 32 + (Xl + Yl)) by ring.
  replace (@unsigned WORDSIZE32 (Int.add (Int.add xh yh) cc) * two_p 32 +
              (Xl + Yl - @unsigned WORDSIZE32 cc * two_p 32))
     with ((@unsigned WORDSIZE32 (Int.add (Int.add xh yh) cc) - @unsigned WORDSIZE32 cc) * two_p 32
           + (Xl + Yl)) by ring.
  apply eqm_add. 2: apply eqm_refl. apply eqm_mul_2p32.
  replace (Xh + Yh) with ((Xh + Yh + @unsigned WORDSIZE32 cc) - @unsigned WORDSIZE32 cc) by ring.
...
Lemma decompose_sub:
  ...
  ofwords (@sub WORDSIZE32 (@sub WORDSIZE32 xh yh) (@sub WORDSIZE32_borrow xl yl (@zero WORDSIZE32)))
          (@sub WORDSIZE32 xl yl).
Proof.
  intros. symmetry. rewrite ofwords_add.
  apply eqm_samerepr.
  rewrite ! ofwords_add'. rewrite (@unsigned WORDSIZE32_sub_borrow xl yl).
  set (bb := @sub WORDSIZE32_borrow xl yl (@zero WORDSIZE32)).
  set (Xl := @unsigned WORDSIZE32 xl); set (Xh := @unsigned WORDSIZE32 xh);
  set (Yl := @unsigned WORDSIZE32 yl); set (Yh := @unsigned WORDSIZE32 yh).
  change Int.modulus with (two_p 32).
  replace (Xh * two_p 32 + Xl - (Yh * two_p 32 + Yl))
     with ((Xh - Yh) * two_p 32 + (Xl - Yl)) by ring.
  replace (@unsigned WORDSIZE32 (@sub WORDSIZE32 (@sub WORDSIZE32 xh yh) bb) * two_p 32 +
              (Xl - Yl + @unsigned WORDSIZE32 bb * two_p 32))
     with ((@unsigned WORDSIZE32 (@sub WORDSIZE32 (@sub WORDSIZE32 xh yh) bb) + @unsigned WORDSIZE32 bb) * two_p 32
           + (Xl - Yl)) by ring.
  apply eqm_add. 2: apply eqm_refl. apply eqm_mul_2p32.
  replace (Xh - Yh) with ((Xh - Yh - @unsigned WORDSIZE32 bb) + @unsigned WORDSIZE32 bb) by ring.
...
Proof.
  ...
  rewrite @sub WORDSIZE32_borrow_add_carry by auto.
  rewrite @sub WORDSIZE32_add_not_3. rewrite Int.xor_assoc. rewrite Int.xor_idem.
  rewrite Int.xor_zero. auto.
  rewrite Int.xor_zero_l. unfold Int.add_carry.
  destruct (zlt (@unsigned WORDSIZE32 xl + @unsigned WORDSIZE32 (Int.not yl) + @unsigned WORDSIZE32 Int.one) Int.modulus);
  compute; [right|left]; apply Int.mkint_eq; auto.
Qed.

Definition mul' (x y: int32) : int64 := repr (@unsigned WORDSIZE32 x * @unsigned WORDSIZE32 y).
...
Proof.
  ...
  set (p := @unsigned WORDSIZE32 x * @unsigned WORDSIZE32 y).
...
- apply eqm_samerepr. apply eqm_add. apply eqm_mul_2p32. auto with ints.
  rewrite @unsigned WORDSIZE32_repr_eq. apply eqm_refl.
Qed.
...
Proof.
  ...
  assert (EQ0: unsigned (mul' xl yl) = @unsigned WORDSIZE32 ph * two_p 32 + @unsigned WORDSIZE32 pl).
  { rewrite <- (ofwords_recompose (mul' xl yl)). apply ofwords_add'. }
  symmetry. rewrite ofwords_add. unfold mul. rewrite !ofwords_add'.
  set (XL := @unsigned WORDSIZE32 xl); set (XH := @unsigned WORDSIZE32 xh);
  set (YL := @unsigned WORDSIZE32 yl); set (YH := @unsigned WORDSIZE32 yh).
  set (PH := @unsigned WORDSIZE32 ph) in *. set (PL := @unsigned WORDSIZE32 pl) in *.
...
Lemma decompose_ltu:
  forall xh xl yh yl,
  ltu (ofwords xh xl) (ofwords yh yl) = if Int.eq xh yh then @ltu WORDSIZE32 xl yl else @ltu WORDSIZE32 xh yh.
Proof.
  intros. unfold ltu. rewrite ! ofwords_add'. unfold @ltu WORDSIZE32, Int.eq.
  destruct (zeq (@unsigned WORDSIZE32 xh) (@unsigned WORDSIZE32 yh)).
  rewrite e. destruct (zlt (@unsigned WORDSIZE32 xl) (@unsigned WORDSIZE32 yl)).
  apply zlt_true; lia.
  apply zlt_false; lia.
  change (two_p 32) with Int.modulus.
  generalize (@unsigned_range WORDSIZE32 xl) (@unsigned_range WORDSIZE32 yl).
  change Int.modulus with 4294967296. intros.
  destruct (zlt (@unsigned WORDSIZE32 xh) (@unsigned WORDSIZE32 yh)).
  apply zlt_true; lia.
  apply zlt_false; lia.
Qed.
...
Lemma decompose_leu:
  ...
  if Int.eq xh yh then negb (@ltu WORDSIZE32 yl xl) else @ltu WORDSIZE32 xh yh.
Proof.
  intros. rewrite decompose_ltu. rewrite Int.eq_sym.
  unfold Int.eq. destruct (zeq (@unsigned WORDSIZE32 xh) (@unsigned WORDSIZE32 yh)).
  auto.
  unfold @ltu WORDSIZE32. destruct (zlt (@unsigned WORDSIZE32 xh) (@unsigned WORDSIZE32 yh)).
  rewrite zlt_false by lia; auto.
  rewrite zlt_true by lia; auto.
Qed.
...
Lemma decompose_lt:
  forall xh xl yh yl,
  lt (ofwords xh xl) (ofwords yh yl) = if Int.eq xh yh then @ltu WORDSIZE32 xl yl else Int.lt xh yh.
Proof.
  intros. unfold lt. rewrite ! ofwords_add''. rewrite Int.eq_signed.
  destruct (zeq (Int.signed xh) (Int.signed yh)).
  rewrite e. unfold @ltu WORDSIZE32. destruct (zlt (@unsigned WORDSIZE32 xl) (@unsigned WORDSIZE32 yl)).
  apply zlt_true; lia.
  apply zlt_false; lia.
  change (two_p 32) with Int.modulus.
  generalize (@unsigned_range WORDSIZE32 xl) (@unsigned_range WORDSIZE32 yl).
...
Lemma decompose_le:
  ...
  if Int.eq xh yh then negb (@ltu WORDSIZE32 yl xl) else Int.lt xh yh.
Proof.
...
Remark int_unsigned_range:
  forall x, 0 <= @unsigned WORDSIZE32 x <= max_unsigned.
Proof.
  intros.
  unfold max_unsigned. unfold modulus.
  generalize (@unsigned_range WORDSIZE32 x).
...
Remark int_unsigned_repr:
  forall x, unsigned (repr (@unsigned WORDSIZE32 x)) = @unsigned WORDSIZE32 x.
Proof.
...
Lemma int_sub_ltu:
  forall x y,
    @ltu WORDSIZE32 x y= true ->
    @unsigned WORDSIZE32 (@sub WORDSIZE32 y x) = unsigned (sub (repr (@unsigned WORDSIZE32 y)) (repr (@unsigned WORDSIZE32 x))).
Proof.
  intros. generalize (@sub WORDSIZE32_ltu x y H). intros. unfold @sub WORDSIZE32. unfold sub.
  rewrite @unsigned WORDSIZE32_repr. rewrite unsigned_repr.
  ...
  lia.
  generalize (@sub WORDSIZE32_ltu x y H). intros.
  generalize (@unsigned_range WORDSIZE32_2 y). intros. lia.
Qed.
...
Global Opaque @repr WORDSIZE32 Int64.repr Byte.repr.
...
Definition to_int (x: int): Int.int := @repr WORDSIZE32 (unsigned x).

Definition to_int64 (x: int): Int64.int := Int64.repr (unsigned x).

Definition of_int (x: Int.int) : int := repr (@unsigned WORDSIZE32 x).
...
Definition agree32 (a: Ptrofs.int) (b: Int.int) : Prop :=
  Ptrofs.unsigned a = @unsigned WORDSIZE32 b.

Lemma agree32_repr:
  forall i, agree32 (Ptrofs.repr i) (@repr WORDSIZE32 i).
Proof.
  intros; red. rewrite Ptrofs.unsigned_repr_eq, @unsigned WORDSIZE32_repr_eq.
...
Proof.
  unfold of_int; intros. rewrite <- (@repr WORDSIZE32_unsigned b) at 2. apply agree32_repr.
Qed.

Lemma agree32_of_ints:
  forall b, agree32 (of_ints b) b.
Proof.
  unfold of_int; intros. rewrite <- (@repr WORDSIZE32_signed b) at 2. apply agree32_repr.
Qed.
...
Proof.
  unfold agree32, to_int; intros. rewrite H. apply @repr WORDSIZE32_unsigned.
Qed.
...
Lemma agree32_sub:
  forall a1 b1 a2 b2,
  agree32 a1 b1 -> agree32 a2 b2 -> agree32 (Ptrofs.sub a1 a2) (@sub WORDSIZE32 b1 b2).
Proof.
  unfold agree32, Ptrofs.sub, @sub WORDSIZE32; intros. rewrite H, H0. apply agree32_repr.
Qed.
...
Proof.
  intros; unfold of_int, to_int. rewrite unsigned_repr. apply @repr WORDSIZE32_unsigned.
  unfold max_unsigned. rewrite modulus_eq32. destruct (@unsigned_range WORDSIZE32 n); lia.
Qed.
...
Proof.
  unfold agree64, Ptrofs.sub, @sub WORDSIZE32; intros. rewrite H, H0. apply agree64_repr.
Qed.
...
Global Hint Resolve
  ...
  @unsigned_range WORDSIZE32 @unsigned_range WORDSIZE32_2
  @repr WORDSIZE32_unsigned @repr WORDSIZE32_signed @unsigned WORDSIZE32_repr : ints.
